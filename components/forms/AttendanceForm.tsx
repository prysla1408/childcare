...
"use client"
import { postAttendance } from "@/backend/controller";
...
function AttendanceForm({ data }: Props) {
  ...
  const action = async (formData: FormData) => {
    const resp = await postAttendance(formData);
    if(resp){
  ...
  return (
    ...
    >
        ...
        <label htmlFor="" className="grid gap-1.5">
            ...
            <input type="datetime-local" name="checkIn" defaultValue={moment(data?.checkIn).format("YYYY-MM-DD hh:mm")} placeholder="Check In" className="px-4 py-2 bg-[#FAF6F2] rounded-full text-primary shadow focus:outline-none focus:ring-4 focus:ring-secondary/40"/>
        </label>

        <label htmlFor="" className="grid gap-1.5">
            <span className="indent-4 font-bold text-base text-primary/80">Check Out </span>
            <input type="datetime-local" name="checkOut" defaultValue={moment(data?.checkOut).format("YYYY-MM-DD hh:mm")} placeholder="Check Out" className="px-4 py-2 bg-[#FAF6F2] rounded-full text-primary shadow focus:outline-none focus:ring-4 focus:ring-secondary/40"/>
        </label>
